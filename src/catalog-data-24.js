/* Sog'liq Industriyasi — app router */
...
const CORP_VIEWS = ["home", "about", "directions", "registration", "tenders", "documents", "news", "cases", "projects", "partners", "licenses", "services", "contacts", "catalog"];
...
const CORP_VIEW_TO_SLUG = {
  ...
  contacts: "contacts", partners: "partners", licenses: "licenses"
};
...
function App() {
  ...
  const [catNav, setCatNav] = useState(_initHash.cat || { sub: "home", param: "", q: "" });
  ...
  const go = (view) => {
    setRoute({ view });
    if (view === "catalog") {setCatNav({ sub: "home", param: "", q: "" });setHashSafe("#/catalog");} else
    setHashSafe(corpHash(view));
    window.scrollTo({ top: 0, behavior: "instant" in window ? "instant" : "auto" });
  };

  /* unified header: jump straight into a catalog sub-view (cart / wishlist / compare / search) */
  const goCat = (sub, param, q) => {
    setCatReady(true);
    setRoute({ view: "catalog" });
    setCatNav({ sub: sub || "home", param: param || "", q: q || "" });
    setHashSafe(q ? "#/catalog" : sub && sub !== "home" && sub !== "listing" ? "#/catalog/" + sub : "#/catalog");
    window.scrollTo({ top: 0, behavior: "instant" in window ? "instant" : "auto" });
  };
  ...
  useEffect(() => {
    ...
    document.title = label ? label + " — " + BRAND : BRAND + " — " + SUB;
  }, [route.view, lang, t]);
  let page;
  const v = route.view;
  ...
  if (v === "partners") page = <PartnersPage t={t} lang={lang} go={go} />;else
      ...
      {catReady && (
        <div className="z-catalog" style={{ display: isCatalog ? "block" : "none" }}>
          <CatalogApp embed={true} active={isCatalog} lang={lang} theme={theme}
            ...
            navSub={catNav.sub} navParam={catNav.param} navQ={catNav.q} />
        </div>
      )}
      {!isCatalog &&
      <React.Fragment>
