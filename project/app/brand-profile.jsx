...
function BrandsListPage({ t, lang, store, go }) {
  ...
  const totalPages = Math.max(1, Math.ceil(filtered.length / BRANDS_PER_PAGE));
  const pageSafe = Math.min(page, totalPages);
  const pageItems = filtered.slice((pageSafe - 1) * BRANDS_PER_PAGE, pageSafe * BRANDS_PER_PAGE);
