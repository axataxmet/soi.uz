    ...
    <footer className="foot">
      <div className="wrap">
        <div className="fcols">
          ...
          <div className="fcol-brand">
            ...
            <ul className="foot-contact">
              ...
              <li className="fc-grp">
                ...
                <span>
                  <a href={telHref(contacts.phone)}>{lv("Приёмная","Qabulxona","Reception")}: {contacts.phone}</a>
                  <a href={telHref(contacts.phone2)}>{lv("Отдел продаж","Sotuv bo'limi","Sales")}: {contacts.phone2}</a>
          ...
          {/* Сворачивание убрано (решение заказчика): списки в футере всегда
              развёрнуты, включая мобильную версию — <details> оставлен как
              семантическая обёртка, но всегда с атрибутом open, без
              стрелки-переключателя. */}
          {footerNavCols(lang, hidden).map((col) => (
            <details key={col.id} className="fcol-acc" open>
              <summary>
                <h5>{col.label}</h5>
              </summary>
              <ul>
