...
function ProductPage({ t, lang, store, go, params }) {
  ...
  /* Близость товара — только по товарной группе (тот же тип оборудования) и
     бренду. Раньше здесь также стояли уровни «та же подкатегория» и «та же
     категория» — на практике оказалось, что это не признак похожести, а
     признак широкого административного деления: дефибриллятор Schiller и
     процедурные камеры для озонотерапии Орион-Си лежат в одной подкатегории
     («Терапевтическое оборудование» — 39 разных товаров), и по одному этому
     совпадению камера считалась «похожей» на дефибриллятор. Категория
     («Медицинское оборудование» — 69 товаров) ещё шире и от неё отказались
     раньше по той же причине. Группа — единственный уровень таксономии,
     который действительно означает «тот же тип изделия». */
  const closeness = (x) => x.group && x.group === p.group ? 2
    : x.brand === p.brand ? 1
    : 0;
  ...
  /* score===0 значит «не связано вообще ничем» (ни группа, ни подкатегория,
     ни категория, ни бренд) — раньше такие товары всё равно попадали в
     список, если совпадений не хватало на 10 штук, и достраивали список
     просто по популярности (дефибриллятор получал в довесок процедурные
     камеры для озонотерапии). Лучше показать меньше товаров, но
     действительно похожих. */
