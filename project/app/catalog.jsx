...
/* «1 наименование / 2 наименования / 5 наименований» — без этого бейдж на
   карточке читается как машинный вывод. Узбекский и английский множественного
   числа здесь не требуют. */
function itemsLabel(n, lang) {
  if (lang === "uz") return n + " ta mahsulot";
  if (lang === "en") return n + (n === 1 ? " item" : " items");
  const d10 = n % 10, d100 = n % 100;
  const w = d10 === 1 && d100 !== 11 ? "наименование"
    : d10 >= 2 && d10 <= 4 && (d100 < 12 || d100 > 14) ? "наименования"
    : "наименований";
  return n + " " + w;
}
...
@media(max-width:800px){ .clp-cats-grid{ grid-template-columns:repeat(2,1fr); } }
