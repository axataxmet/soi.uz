...
/* Фирменный водяной знак раньше накладывался только CSS-ом поверх готовой
   ...
   открытый напрямую по URL) знака не содержал вовсе. Поэтому запекаем его в
   файл на этапе загрузки: рисуем оригинал на канвасе, поверх — тот же PNG
   (прозрачность 20% заложена в сам файл).

   Знак стоит ПО ЦЕНТРУ и занимает 30% меньшей стороны (решение заказчика от
   15.09.2026; весь каталог приведён к этому виду разом). Раньше он ставился
   в правый верхний угол — если оставить старую геометрию, каждое новое фото
   выбивалось бы из каталога. 30%, а не больше: в плитке каталога картинка
   показывается вчетверо мельче, и крупный знак перекрывал бы дисплеи и
   шильдики, по которым покупатель сверяет модель. */
