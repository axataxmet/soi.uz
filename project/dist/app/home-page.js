...
.z-corp .sx [class*="-ctaband"] h1, .z-corp .sx [class*="-ctaband"] h2, .z-corp .sx [class*="-ctaband"] h3 { color:inherit; }
...
/* Карточка приведена к виду карточки каталога (.sc-card на
   /catalog/equipment) — по прямому запросу заказчика 13.09.2026: белая
   карточка, тень вместо цветной заливки, без крупного номера и без строки
   «Подробнее». Раньше при наведении вся карточка заливалась цветом (лайм) —
   теперь только тень приподнимает карточку и заголовок синеет, как у
   .sc-card на странице каталога. */
.sxp-card { position:relative; display:flex; flex-direction:column; overflow:hidden; text-align:left;
  ...
  box-shadow:var(--sh);
  transition:flex-grow .5s ease, box-shadow .25s ease, transform .25s ease, border-color .25s ease; }
@media(min-width:1024px){ .sxp-card { flex:1 1 0; } .sxp-card:hover { flex-grow:1.35; } }
.sxp-card:hover { box-shadow:var(--sh-lg); transform:translateY(-3px); border-color:var(--sx-line); }
.sxp-card:hover .sxp-t { color:var(--blue-600); }
...
/* Та же подсветка контура, что убрана у «Реализованные проекты» и «Новости»
   (решение заказчика 13.09.2026) — на фото-карточках «Электронный каталог»
   рамка тем более не видна под фотографией, но правило снято для
   единообразия со всеми остальными карточками главной. */
.sxc-card.ov:hover { border-color:var(--sx-line); }
...
@media (prefers-reduced-motion: reduce){
  .sxc-card, .sxc-media img, .sxc-arr,   .sxc-card:hover .sxc-media img { transform:none; }
    ...
    `;document.head.appendChild(s)},[]);var goCard=card=>{var found=cats.find(c=>c.slug===card.catKey||c.id===card.catKey);go("catalog",found?{cat:found.id}:{})};return React.createElement("section",{className:"sxc"},React.createElement("div",{className:"sxc-inner"},React.createElement("div",{className:"sxc-head sx-rv"},React.createElement("div",null,React.createElement("p",{className:"sxc-kicker"},_lv(lang,"Электронный каталог","Elektron katalog","Digital catalog")),React.createElement("h2",{className:"sxc-h2 sx-h2-link",onClick:()=>go("catalog")},_lv(lang,"Оборудование для современной медицины","Zamonaviy tibbiyot uchun uskunalar","Equipment for modern medicine"))),React.createElement("div",null,React.createElement("p",{className:"sxc-sub"},_lv(lang,"Структурированный каталог решений для диагностики, лечения, реанимации и ежедневной работы медицинских учреждений.","Tibbiyot muassasalarining diagnostika, davolash, reanimatsiya va kundalik ish uchun yechimlar katalogi.","A structured catalog of solutions for diagnostics, treatment, intensive care and the daily work of medical institutions.")))),React.createElement("div",{className:"sxc-grid"},CATALOG_CARDS.map((card,i)=>React.createElement("a",{key:card.slug,className:"sxc-card ov sx-rv",style:{"--i":i},href:"/catalog/"+card.slug,onClick:e=>{if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey||e.button!==0)return;e.preventDefault();goCard(card)}},React.createElement("div",{className:"sxc-media"},React.createElement("img",{src:window.__asset("assets/catalog/"+card.slug+".jpg"),alt:"",loading:"lazy"})),React.createElement("div",{className:"sxc-foot"},React.createElement("h3",{className:"sxc-t"},_lv(lang,card.t.ru,card.t.uz,card.t.en)),React.createElement("div",{className:"sxc-bottom"},React.createElement("div",{className:"sxc-more"},_lv(lang,"Подробнее","Batafsil","Read more"),React.createElement("span",{className:"sxc-arr","aria-hidden":true},"↗")),React.createElement("span",{className:"sxc-bignum","aria-hidden":true},String(i+1).padStart(2,"0")))))))))}function SoiDirections({lang,go}){var DD=window.DIRECTIONS_DATA;if(!DD)return null;var{DIRECTION_GROUPS,getDirsForGroup,getProductsForDir}=DD;var P=window.DATA&&window.DATA.PRODUCTS||[];var dirCards=DIRECTION_GROUPS.map(g=>{var dirsAll=getDirsForGroup(g.id);var dirsWithCounts=dirsAll.map(d=>({d,cnt:getProductsForDir(d.id,P).length})).filter(x=>x.cnt>0);return{g,dirs:dirsWithCounts.slice(0,4)}}).filter(x=>x.dirs.length>0);if(dirCards.length===0)return null;return React.createElement("section",{className:"sxc"},React.createElement("div",{className:"sxc-inner"},React.createElement("div",{className:"sxc-head sx-rv"},React.createElement("div",null,React.createElement("p",{className:"sxc-kicker"},_lv(lang,"Навигация по направлениям","Yo'nalishlar bo'yicha","By specialty")),React.createElement("h2",{className:"sxc-h2"},_lv(lang,"Подбор по направлению медицины","Tibbiyot yo'nalishi bo'yicha tanlov","Find by medical specialty"))),React.createElement("div",null,React.createElement("p",{className:"sxc-sub"},_lv(lang,"Откройте каталог по профилю учреждения, отделению или клинической задаче.","Muassasa profili yoki klinik vazifa bo'yicha katalogni oching.","Open the catalog by institution profile, department or clinical task.")))),React.createElement("div",{className:"sx-dir-grid"},dirCards.map(({g,dirs},i)=>{return(React.createElement("div",{className:"sx-dir sx-rv",key:g.id,style:{"--i":i}},React.createElement("div",{className:"sx-dir-ic"},React.createElement(Icon,{name:g.icon,size:39})),React.createElement("h3",null,React.createElement("a",{className:"sx-dir-t",href:"/catalog",onClick:e=>{if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey||e.button!==0)return;e.preventDefault();go("catalog",{dir:dirs[0]&&dirs[0].d.id})}},_lv(lang,g.ru,g.uz,g.en))),React.createElement("div",{className:"sx-dir-links"},dirs.map(({d,cnt})=>React.createElement("a",{key:d.id,href:"/catalog",onClick:e=>{if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey||e.button!==0)return;e.preventDefault();go("catalog",{dir:d.id})}},_lv(lang,d.ru,d.uz,d.en)," ",React.createElement("span",{className:"sx-dir-cnt"},cnt)))),React.createElement("div",{className:"sx-dir-more"},_lv(lang,"Подробнее","Batafsil","Read more"),React.createElement("span",{className:"sx-dir-arrow","aria-hidden":"true"},"↗"),React.createElement("span",{className:"sx-dir-bignum","aria-hidden":"true"},String(i+1).padStart(2,"0")))))}))))}function SoiCountUp({value}){var ref=useRef(null);var[disp,setDisp]=useState("0");useEffect(()=>{var target=parseInt(String(value).replace(/\s/g,""),10);if(isNaN(target)){setDisp(value);return}var fmt=n=>n.toLocaleString("ru-RU").replace(/,/g," ");var done=false;var run=()=>{var start=performance.now(),DUR=1500;var step=now=>{var p=Math.min((now-start)/DUR,1);var e=1-Math.pow(1-p,3);setDisp(fmt(Math.round(e*target)));if(p<1)requestAnimationFrame(step)};requestAnimationFrame(step)};var io=new IntersectionObserver(([en])=>{if(en.isIntersecting&&!done){done=true;run()}},{threshold:0.4});if(ref.current)io.observe(ref.current);return()=>io.disconnect()},[value]);return React.createElement("span",{ref:ref},disp)}var PARTNERS_MIN=4;var PARTNERS_FALLBACK=[{id:"ph",name:"Philips"},{id:"am",name:"Армед"},{id:"kpz",name:"Касимовский ПЗ"},{id:"md",name:"Midmark"},{id:"ns",name:"Нейрософт"},{id:"dr",name:"Dräger"},{id:"mn",name:"Mindray"},{id:"el",name:"Елатомский ПЗ"}];function SoiBrands({lang,go}){var brands=(window.DATA&&window.DATA.BRANDS||[]).filter(b=>b&&b.name);var items=brands.length>=PARTNERS_MIN?brands:PARTNERS_FALLBACK;var half=Math.ceil(items.length/2);var rows=items.length>1?[items.slice(0,half),items.slice(half)]:[items,items];var item=(b,row,dup)=>React.createElement("button",{className:"sx-mq-item",key:row+(dup?"b-":"a-")+b.id,onClick:()=>go("partners"),tabIndex:dup?-1:0,title:_lv(lang,"Все партнёры","Barcha hamkorlar","All partners")},b.logo?React.createElement("img",{src:b.logo,alt:b.name,loading:"lazy",onError:e=>{e.currentTarget.replaceWith(document.createTextNode(b.name))}}):b.name,b.flag?React.createElement("span",{className:"sx-mq-flag"},b.flag):null);var belt=(list,row)=>React.createElement("div",{className:"sx-mq-row",key:row},React.createElement("div",{className:"sx-mq-track"+(row?" rev":""),style:{"--mq-dur":Math.max(22,list.length*5.5)+"s"}},React.createElement("div",{className:"sx-mq-pass"},list.map(b=>item(b,row,false))),React.createElement("div",{className:"sx-mq-pass","aria-hidden":"true"},list.map(b=>item(b,row,true)))));return React.createElement("section",{className:"sx-mq-sec"},React.createElement("div",{className:"sx-mq-head"},React.createElement("div",{className:"sxc-h2-row"},React.createElement("h2",{className:"sx-h2 sx-brands-title sx-rv sx-h2-link",onClick:()=>go("partners"),style:{margin:0}},_lv(lang,"Партнёры","Hamkorlar","Partners")),React.createElement("button",{type:"button",className:"sxc-act sxc-act-all",onClick:()=>go("partners")},_lv(lang,"Все партнёры","Barcha hamkorlar","All partners"),React.createElement(Icon,{name:"arrowRight",size:16})))),React.createElement("div",{className:"sx-mq-vp"},React.createElement("div",{className:"sx-mq-fade"}),rows.map((list,i)=>belt(list,i))))}function ensureCaseModalCss(){if(document.getElementById("sx-cmod-css"))return;var s=document.createElement("style");s.id="sx-cmod-css";s.textContent=`
